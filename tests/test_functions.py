...
from tests.fixtures import SAMPLE_TRACK, SAMPLE_PLAYLIST, SAMPLE_USER, ctx_with_token
...
async def test_search_tracks_returns_formatted_tracks():
    ...
    ctx.api.mock_get("api.spotify.com/v1/search", {"tracks": {"items": [SAMPLE_TRACK]}})
...
async def test_search_tracks_empty_results():
    ctx = await ctx_with_token()
    ctx.api.mock_get("api.spotify.com/v1/search", {"tracks": {"items": []}})
...
async def test_search_tracks_rate_limit_is_retryable():
    ctx = await ctx_with_token()
    ctx.api.mock_get("api.spotify.com/v1/search", {}, status=429)
    result = await fn_search_tracks(ctx, SearchTracksParams(query="test"))
    assert result.status == "error"
...
async def test_get_playlists_returns_list():
    ctx = await ctx_with_token()
    ctx.api.mock_get("api.spotify.com/v1/me/playlists", {"items": [SAMPLE_PLAYLIST]})
...
async def test_create_playlist_success():
    ctx = await ctx_with_token()
    ctx.api.mock_get("api.spotify.com/v1/me", SAMPLE_USER)
    ctx.api.mock_post("api.spotify.com/v1/users/spotify_user_123/playlists", SAMPLE_PLAYLIST)
...
async def test_add_track_to_playlist_success():
    ctx = await ctx_with_token()
    ctx.api.mock_post("api.spotify.com/v1/playlists/37i9dQZF1DXcBWIGoYBM5M/tracks", {})
...
async def test_remove_track_from_playlist_success():
    ctx = await ctx_with_token()
    ctx.api._mocks.append(("DELETE", "api.spotify.com/v1/playlists/37i9dQZF1DXcBWIGoYBM5M/tracks", {}, 200))
...
async def test_get_recent_tracks_premium_required():
    ctx = await ctx_with_token()
    ctx.api.mock_get("api.spotify.com/v1/me/player/recently-played", {}, status=403)
    result = await fn_get_recent_tracks(ctx, GetRecentTracksParams())
    assert result.status == "error"
...
async def test_like_track_success():
    ctx = await ctx_with_token()
    ctx.api._mocks.append(("PUT", "api.spotify.com/v1/me/tracks", {}, 200))
...
async def test_unlike_track_success():
    ctx = await ctx_with_token()
    ctx.api._mocks.append(("DELETE", "api.spotify.com/v1/me/tracks", {}, 200))
